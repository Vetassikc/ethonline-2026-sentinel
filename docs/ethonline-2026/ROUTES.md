...
- `position-evidence.ts`: source response -> normalized envelope/hash; intent + evidence + policy config -> decision.
...
- `scripts/position-evidence.ts`: narrow tool entry point used by an AI client; JSON output, no arbitrary URLs or execution capability.
